...
- **전 세계 벡터 지도:** MapLibre GL JS와 OpenFreeMap Liberty 벡터 타일 (토큰 불필요)
...
- **베이스맵 언어 연동:** 사이트에서 선택한 언어에 맞춰 지도 라벨 변경 (OpenMapTiles 다국어 데이터)
...
본 프로젝트는 무료 OpenFreeMap 타일을 사용하므로 등록이나 토큰이 필요하지 않습니다.
...
# 선택 사항: 기본 지도 스타일 덮어쓰기 (기본값은 OpenFreeMap Liberty)
# NEXT_PUBLIC_MAP_STYLE_URL=https://tiles.openfreemap.org/styles/liberty
```

| 변수 | 필수 | 설명 |
| --- | --- | --- |
| `NEXT_PUBLIC_MAP_STYLE_URL` | 아니요 | 사용자 정의 MapLibre 호환 스타일 URL. 설정하지 않으면 OpenFreeMap Liberty 사용 |
| `SITE_URL` | 프로덕션 | Canonical, hreflang, 사이트맵, 소셜 메타데이터에 사용되는 공개 사이트 URL |
...
지도 라벨은 사이트 언어에 따라 동적으로 전환되며, OpenMapTiles 다국어 필드(`name:ja`, `name:en`, `name:zh`, `name:ko`)를 사용하고 해당 언어가 없으면 현지 이름으로 폴백합니다.

UI 텍스트는 `src/i18n/messages.ts`, 검색 메타데이터는 `src/i18n/seo.ts`에 집중되어 있습니다. 사용자 대상 또는 검색 대상 텍스트를 추가하거나 변경할 때는 6개 언어를 모두 업데이트하세요. 각 언어에는 개별 색인 가능한 URL(`/en`, `/ja`, `/zh-CN`, `/zh-TW`, `/zh-HK`, `/ko`)이 있으며 hreflang으로 상호 연결됩니다. 사용자가 언어를 선택하지 않으면 루트 URL은 브라우저 언어를 따릅니다. 수동으로 선택한 언어는 LocalStorage와 Cookie에 저장되어 이후 방문 시 복원됩니다.

지도는 무료 OpenFreeMap 타일을 사용합니다. 지도 오른쪽 아래의 OpenStreetMap 저작자 표시는 제거하거나 가리지 마세요.
